...
downloads_path = "/data/csc4801/KedzioraLab/thingiverse/stairs/" 
...
api_token = "21a840b95f490739cd72125726eb3a8c"
...
def relevant(keywords, n_pages=1):
    generic_search(keywords, sort_type="relevant", n_pages=n_pages)

def text(keywords, n_pages=1):
    generic_search(keywords, sort_type="text", n_pages=n_pages)

def popular(keywords, n_pages=1):
    generic_search(keywords, sort_type="popular", n_pages=n_pages)

def makes(keywords, n_pages=1):
    generic_search(keywords, sort_type="makes", n_pages=n_pages)

def newest(keywords, n_pages=1):
    generic_search(keywords, sort_type="newest", n_pages=n_pages)
...
def download_objects(rest_url, file_name, mode = "none"):
    ...
    for object in range(len(data_pd)):
        ...
        file_path = os.path.join(stl_path, data_pd[object]["name"].replace(" ", "_").replace("/", "-").replace(",", "").replace("|", "")
        ...
        tags_r = s.get(thingiverse_api_base+rest_keywords["things"] + object_id + rest_keywords["tags"] + access_keyword + api_token)
        thing_r = s.get(thingiverse_api_base+rest_keywords["things"] + object_id + access_keyword + api_token)
        ...
        try:
            ...
            print("No files for this object")


        try:
            tags_info = tags_r.json()
        except:
            print("No tags for this object")
            
        try:
            with open(file_path+"/"+"DESCRIPTIONS.txt", "wb") as text:
                text.write(thing_r.json()["description"].encode('utf-8'))
        except:
            print("No description for this object")
        
        try:
            with open(file_path+"/"+"TAGS.txt", "wb") as text:
...
if __name__ == "__main__":
    ...
    print("University of Cambridge and Thomas Benzshawel")
    ...
    parser.add_argument("--relevant", type=bool, dest="relevant_true",
                        help="It takes the most relevant objects uploaded")
    ...
    if args.keywords:
        generic_search(args.keywords, n_pages=args.pages)
    elif args.newest_true:
        newest(args.pages)
    elif args.popular_true:
        popular(args.pages)
    elif args.username:
        user(args.username, args.pages)
    elif args.likes:
        likes(args.likes, args.pages)
    elif args.relevant_true:
        relevant(args.relevant_true)
    else:
        newest(args.pages)