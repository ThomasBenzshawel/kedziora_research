...
def main():
    ...
    for file_path in description_files:
        try:
            ...
            # Extract category from Label line
            category = "Uncategorized"  # Default category
            
            # Parse for "Label: " followed by the category name
            lines = description.split('\n')
            for i, line in enumerate(lines):
                if line.startswith("Label:"):
                    # Extract the category from the Label line
                    category_text = line[6:].strip()  # Remove "Label: " prefix and whitespace
                    
                    # Use the category if it's not empty
                    if category_text:
            ...
            if existing_object:
                ...
                result = objects_collection.update_one(
                    {"objectId": object_id},
                    {
                        "$set": {
                            ...
                            "category": category,  # Now using the extracted category
                            "updatedAt": now
                        }
                    }
                )
                ...
                if result.modified_count > 0:
                    print(f"Updated description for object: {object_id} (Category: {category})")
                    updated_count += 1
                else:
                ...
                print(f"Created new object: {object_id} (Category: {category})")
                created_count += 1
