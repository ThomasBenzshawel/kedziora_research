...
def generate_color_image(mesh, rotation_matrix):
    ...
    try:
        ...
        # Check for NaN and infinite values in depth array
        depth_is_nan = np.isnan(depth)
        depth_is_inf = np.isinf(depth)

        # Replace NaN and infinite values with 0
        depth[depth_is_nan] = 0
        depth[depth_is_inf] = 0
    ...
    try:
        # Check for NaN and infinite values in depth_float array
        depth_float_is_nan = np.isnan(depth_float)
        depth_float_is_inf = np.isinf(depth_float)

        # Replace NaN and infinite values with 0
        depth_float[depth_float_is_nan] = 0
        depth_float[depth_float_is_inf] = 0

        depth_float = np.clip(depth_float, 0.02, 1)  # Clip values to the range [0, 1]

        depth_scaled = 0.8 * (depth_float - depth_float.min()) / depth_float.ptp() + 0.2
        depth_scaled = np.nan_to_num(depth_scaled, nan=0)  # Replace NaN values with 0
    except Exception as e:
    ...
    return np.array(tmp1, dtype=np.float64) / -255 + 1
...
def generate_depth_map(mesh, rotation_matrix):
    ...
    try:
        ...
        print(f"Error generating base depth TWO: {e}")
        return a
    ...
    try:
        ...
        depth_range = np.maximum(depth_range, 1e-8)  # Add a small epsilon to avoid division by zero
        depth_float = (depth - depth_min) / depth_range
    except Exception as e:
        print(f"Error generating depth range: {e}")
        return a
    ...
    try:
        ...
        depth_float = np.clip(depth_float, 0.02, 1)
    except Exception as e:
        print(f"Error generating depth float TWO: {e}")
        return a

    try:
        depth_scaled = 0.8 * (depth_float - np.min(depth_float)) / np.ptp(depth_float) + 0.2
        depth_scaled[np.isnan(depth_scaled)] = 0  # Set NaN values to 0
    except Exception as e:
        print(f"Error generating depth scaled: {e}")
        return a
...
# %%
def generate_stuff(class_name, stl_file, dpth_dir, dpth_fixed_dir, img_dir, gt_dir, obj_dir, eight_point):
    print(f'Processing FILE: {stl_file}')
    filename = stl_file.split('.')[0].split('/')[-1]
    ret = []

    obj_file_path = os.path.join(obj_dir, f'{filename}.obj')
    ...
    for z in range(3):
        ...
        while not is_valid_point(mesh, quaternionToRotMatrix(point)):
            point = randomRotation()
        mesh.apply_transform(quaternionToRotMatrix(point))
        filename = filename + "_" + str(z)

        # class_depth
        dpth_file_path = os.path.join(dpth_dir, f'{filename}.mat')
        # class_depth_fixed8
        dpth_fixed_file_path = os.path.join(dpth_fixed_dir, f'{filename}.mat')
        # class_inputRGB
        img_file_path = os.path.join(img_dir, f'{filename}.npy')
        # class_testGT
        testGT_file_path = os.path.join(gt_dir, f'{filename}.mat')

        try:
            depths = {"Z": [], "trans": []}
            depths_fixed = {"Z": []}
            images = []
            # print(obj_file_path, "densify_shape")
            testGT = densify_shape(obj_file_path)
        except Exception as e:
            print(f"Error densifying - {stl_file}: {e}")

        try:
            for point in eight_point:
                depths_fixed["Z"].append(generate_depth_map(mesh.copy(), point))
        except Exception as e:
            print(f"Error generating depth fixed - {stl_file}: {e}")
        ...
        try:
            for i in range(24):
                ...
                images.append(generate_color_image(mesh.copy(), quaternionToRotMatrix(point)))
        except Exception as e:
        ...
        try:
            for i in range(100):
                ...
                while not is_valid_point(mesh, quaternionToRotMatrix(point)):
                    point = randomRotation()
                depths["trans"].append(point)
                depths["Z"].append(generate_depth_map(mesh.copy(), quaternionToRotMatrix(point)))
        except Exception as e:
            print(f"Error generating depth({err_index}) - {stl_file}: {e}")

        try:
            scipy.io.savemat(dpth_file_path, depths)
            scipy.io.savemat(dpth_fixed_file_path, depths_fixed)
            np.save(img_file_path, np.array(images, dtype=np.float64))
            scipy.io.savemat(testGT_file_path, testGT)
            print(f"Saved conversion - {stl_file}")
            ret.append(class_name + "/" + filename)
        except Exception as e:
            print(f"Error saving conversion - {stl_file}: {e}")
    return ret
...
def main():
    ...
    base_directory = '/data/csc4801/KedzioraLab/fixed_thingiverse/'
    save_directory = '/data/csc4801/KedzioraLab/TrainingData/Thingiverse12/'
    ...
    for category_name in os.listdir(base_directory):
        ...
        if os.path.isdir(category_path):
            for train_test in os.listdir(category_path):
                dpth_dir = os.path.join(training_path, category_name + '_depth')
                dpth_fixed_dir = os.path.join(training_path, category_name + '_depth_fixed8')
                img_dir = os.path.join(training_path, category_name + '_inputRGB')
                gt_dir = os.path.join(training_path, category_name + '_testGT')
                lst_dir = os.path.join(training_path, category_name + '_' + train_test + ".list")
                ...
                if not os.path.exists(dpth_fixed_dir):
                    os.makedirs(dpth_fixed_dir)
                ...
                for stl_file in glob.glob(stl_files_pattern):
                    args.append(
                        [category_name, stl_file, dpth_dir, dpth_fixed_dir, img_dir, gt_dir, obj_dir, eight_point])
                ...
                with mp.Pool(processes=8) as p:  # only 8 processes because thats how many we requested
                    lst = list(p.starmap(generate_stuff, args))
                    p.close()
                    p.join()

                with open(lst_dir, 'w') as f:
                    for item in lst:
                        for i in item:
                            f.write("%s\n" % i)
