...
class OFFUtils:
    ...
    def loadOFF(self, infilename):
        try:
            with open(infilename, 'r') as f:
                lines = f.readlines()
            (np, nf, _) = list(map(int, lines[1].split()))

            pts = [list(map(float, l.split())) for l in lines[2:2+np]]
            self.triangles = [[pts[p] for p in map(int,l.strip().split()[1:])] for l in lines[2+np:]]
    ...
    def volume(self, unit, material_mass):
        """ volume in cm^3 """
        return sum(self.signedVolumeOfTriangle(p1, p2, p3) for p1, p2, p3 in self.triangles) / 1000

    def mass(self, unit, material_mass):
        return self.volume(unit,material_mass) * material_mass
...
class STLUtils:
    ...
    def surf_area(self):
        ...
        # areaCm2 = area / 100
        self.areas = list(map(self.areaOfTriangle, self.triangles))
        areaCm2 = sum(self.areas) / 100
        print("Total area:", areaCm2, "cm^2")
        return areaCm2
...
def main():
    ...
    parser.add_argument('--filetype', choices=['stl', 'off', 'nii', 'dcm'], default='stl', help='Type of the input file: stl, nii, dcm')
    ...
    elif args.filetype == 'off':
        myOFFUtils = OFFUtils()
        myOFFUtils.loadOFF(args.filename)
        if args.calculation == 'volume':
            material_mass = materialsFor3DPrinting().get_material_mass(args.material)
            myOFFUtils.calculateVolume(args.unit, material_mass)
        elif args.calculation == 'area':
            myOFFUtils.surf_area()
